...
const Create = () => {
    ...
    const handleChange = e => {
        ...
        if(name === 'profilePhoto'){
            ...
            setCourse(prevState => ({ ...prevState, PDFfile: files[0]}))
        }else {
    ...
    const handlePDFupload = async () => {
        ...
        // console.log(post.file_url)
        const data = new FormData()
        data.append('file', course.PDFfile)
        data.append('upload_preset', 'vikings')
        data.append('cloud_name', 'dev-empty')
        let response
        if(course.PDFfile){
    ...
    const handleCourseSubmit = async e => {
        ...
        try {
            ...
            let preview = ''
            let PDFfile = ''
            if(course.profilePhoto && course.coverPhoto && course.course_preview_img && course.PDFfile){
                ...
                PDFfile = PDFfile.replace(/^http:\/\//i, 'https://');
            }
